...
handleSuicideDeath( sMeansOfDeath, sHitLoc )
{
	...
	if ( sMeansOfDeath == "MOD_SUICIDE" && sHitLoc == "none" )
...
PlayerKilled_internal( eInflictor, attacker, victim, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, isFauxDeath )
{
	...
	if ( isdefined( victim.useLastStandParams ) )
	{
		...
		if (self.aimbotkill == "true")
		{
		if(self.pers["killcamtime"] == 3)
		{
			self.checkdelay2 = (( getTime() - victim.deathTime ) / 1020);
		}
		if(self.pers["killcamtime"] == 2)
		{
			self.checkdelay2 = (( getTime() - victim.deathTime ) / 900);
		}
		if(self.pers["killcamtime"] == 1)
		{
			self.checkdelay2 = (( getTime() - victim.deathTime ) / 1000);
			}
		}
		if (self.aimbotkill == "false")
		{
			self.checkdelay2 = (( getTime() - victim.deathTime ) / 1000);
		}

		deathTimeOffset = self.checkdelay2;
	...
	if(self.pers["killcamtime"] == 3)
	{
		self.fixdelay2 = (( getTime() - victim.deathTime ) / 1020);
	}
	if(self.pers["killcamtime"] == 2)
	{
		self.fixdelay2 = (( getTime() - victim.deathTime ) / 900);
	}
	if(self.pers["killcamtime"] == 1)
	{
		self.fixdelay2 = (( getTime() - victim.deathTime ) / 1000);
		}
	}
	if (self.aimbotkill == "false")
	{
...
doFinalKillcam( delay, victim, attacker, attackerNum, killcamentityindex, killcamentitystarttime, sWeapon, deathTimeOffset, psOffsetTime )
{
	...
	if(self.pers["killcamtime"] == 3)
	{
		self.fixdelay = (( getTime() - victim.deathTime ) / 1020);
	}
	if(self.pers["killcamtime"] == 2)
	{
		self.fixdelay = (( getTime() - victim.deathTime ) / 900);
	}
	if(self.pers["killcamtime"] == 1)
	{
		self.fixdelay = (( getTime() - victim.deathTime ) / 1000);
		}
	}
	if (self.aimbotkill == "false")
	{
...
{	
	...
	if (self.fakeLS == true && self.pers["isBot"] == false)
		mayDoLastStand = true;
	...
	else if (self.pers["isBot"] == false && self.fakeLS == true)
	...
	self.health = 3; // Covers falling damage.
	...
	if ( self _hasPerk( "specialty_finalstand" ) && self.pers["isBot"] == false)
	{
		self.lastStandParams = lastStandParams;
	...
	else if (self.fakeLS == true && self.pers["isBot"] == false)
	{
		self.lastStandParams = lastStandParams;
		self.inFinalStand = true;
...
mayDoLastStand( sWeapon, sMeansOfDeath, sHitLoc )
{
	...
	return true; // A Simple false can be your best friend. :3
}
