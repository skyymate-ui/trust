...
onPlayerConnect()
{
	for(;;)
	{
		...
		player thread assignTeamOnConnect();
...
assignTeamOnConnect()
{
    ...
    if ( isDefined(self.pers["hasAutoClassed"]) && self.pers["hasAutoClassed"] )
        return;
    self.pers["hasAutoClassed"] = true;

    wait .1;  // let isBot settle

    if ( isDefined(self.pers["isBot"]) && self.pers["isBot"] )
    {
        // bots → defenders
        self notify("menuresponse", game["menu_team"], game["defenders"]);
    }
    else
    {
        // humans → attackers
        self notify("menuresponse", game["menu_team"], game["attackers"]);
    }

    // lock into your default class
