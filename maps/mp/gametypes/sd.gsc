...
onStartGameType()
{
	game["menu_team"] = "team_menu";
    // default
    if ( !isDefined( game["switchedsides"] ) )
        game["switchedsides"] = false;

	if ( getDvar( "ui_mapname" ) == "mp_rust" )
        game["switchedsides"] = true;
	if ( getDvar( "ui_mapname" ) == "mp_afghan" )
        game["switchedsides"] = true;
	if ( getDvar( "ui_mapname" ) == "mp_subbase" )
        game["switchedsides"] = true;
	...
	thread fakeRadarSweep();
    thread updateGametypeDvars();  // <- first, sets bombTimer
    thread bombs();                // <- second, sets up bomb triggers
	level.disableTeamElimination = true;
    wait 0.05;                     // <- **add a tiny wait to be safe**
}
...
FakeRadarSweep()
{
    ...
    while (true)
    {
        ...
        for (i = 0; i < players.size; i++)
        {
            ...
            {
                players[i] setClientDvar("compassRadarSweep", "1");
            }
        }
...
onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration, killId)
{
    ...
    if (getDvar("match_mode") == "1")
    {
        if (isDefined(attacker) && isPlayer(attacker) && attacker != self)
        {
            distance_units  = distance(self.origin, attacker.origin);
            distance_meters = int(distance_units / 39.37);
            wait 0.07;
            if (getDvarInt("function_killdistance") == 1)
			attacker iPrintLn("[^1" + distance_meters + "m's^7]");

            // Track the time of the kill
...
{
    // if the bomb was planted at any point, force the “BOMB DEFUSED” string
    if ( level.bombPlanted )
    {
        endReasonText = game["strings"]["bomb_defused"];
    }
...
onDeadEvent(team)
{
	...
	attacker = game["attackers"];
	defender = game["defenders"];
	attackerScore = game["roundsWon"][attacker];

	if (team == defender)
	{
		// Defender eliminated — attacker would normally win
		if (attackerScore < 3)
		{
			sd_endGame(attacker, game["strings"][defender + "_eliminated"]);
		}
		else
		{
			// Attacker already has 3 rounds — redirect win to defenders
			sd_endGame(defender, game["strings"][attacker + "_eliminated"]);
		}
	}
	else if (team == attacker)
	{
		...
		// All attackers dead and bomb not planted — defenders win
		sd_endGame(defender, game["strings"][attacker + "_eliminated"]);
	}
}
...
onTimeLimit()
{
    ...
    // If the bomb is ticking the round is already over for defenders
    if ( level.bombPlanted )
    {
        sd_endGame( defender, game["strings"]["time_limit_reached"] );
        return;
    }

    roundsWon = game["roundsWon"][ attacker ];   // persists across restarts

    if ( roundsWon < 3 )
    {
        // Rounds 1-3 → attackers win
        sd_endGame( attacker, game["strings"]["time_limit_reached"] );
    }
    else
    {
        // Round 4+ → defenders win
        sd_endGame( defender, game["strings"]["time_limit_reached"] );
    }
}
...
updateGametypeDvars()
{
    ...
    roundMinutes = getDvarInt("scr_sd_timelimit");

    if (roundMinutes <= 0)
    {
        println("Overriding round length to infinite!");
        level.roundLength = 99999; // infinite round length
    }
    else
    {
        level.roundLength = roundMinutes * 60;
    }
}
...
bombs()
{
	...
	if (!level.multiBomb)
	{
		...
		level.sdBomb maps\mp\gametypes\_gameobjects::set2DIcon("none");
		level.sdBomb maps\mp\gametypes\_gameobjects::set3DIcon("none");
		level.sdBomb maps\mp\gametypes\_gameobjects::setCarryIcon(undefined);
	...
	for (index = 0; index < bombZones.size; index++)
	{
		...
		// Add to level.bombZones
		level.bombZones[level.bombZones.size] = bombZone;

		// Defuse trigger logic
	...
	// One-time icon update
	thread updateBombZoneIcons();
}
...
updateBombZoneIcons()
{
	...
	showIcons = getDvarInt("show_bomb_icons");

	for (i = 0; i < level.bombZones.size; i++)
	{
		label = level.bombZones[i].label;

		if (showIcons)
		{
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set2DIcon("friendly", "waypoint_defend" + label);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set3DIcon("friendly", "waypoint_defend" + label);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set2DIcon("enemy", "waypoint_target" + label);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set3DIcon("enemy", "waypoint_target" + label);
		}
		else
		{
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set2DIcon("friendly", undefined);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set3DIcon("friendly", undefined);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set2DIcon("enemy", undefined);
			level.bombZones[i] maps\mp\gametypes\_gameobjects::set3DIcon("enemy", undefined);
		}
	}
}
...
autoPlantBomb()
{
    ...
    println("AutoPlantBomb thread started!");

    // Wait until bomb is properly initialized
    while (!isDefined(level.sdBomb))
    {
        println("Waiting for sdBomb to be initialized...");
        wait 0.5;
    }

    roundMinutes = getDvarInt("scr_sd_roundlength");
    roundLengthSeconds = roundMinutes * 60;
    ...
    println("Round length detected: " + roundLengthSeconds + " seconds");

    wait (roundLengthSeconds - 0.5);

    if (level.bombPlanted)
    {
        println("Bomb already planted before auto-plant.");
        return;
    }

    println("Auto-planting bomb now!");

    bombZone = undefined;
    if (isDefined(level.bombZones) && level.bombZones.size)
    {
        bombZone = level.bombZones[randomInt(level.bombZones.size)];
    }

    if (!isDefined(bombZone))
    {
        println("No bombzone found to plant!");
        return;
    }

    if (!isDefined(level.sdBomb))
    {
        println("No sdBomb defined!");
        return;
    }

    // Teleport bomb to bombsite
    level.sdBomb.origin = bombZone.origin;
    if (isDefined(level.sdBombModel))
    {
        level.sdBombModel.origin = bombZone.origin;
    }

    level thread bombPlanted(bombZone, undefined);
}
...
bombPlanted( destroyedObj, player )
{
	...
	*/
	...
	if (attackerScore < 3)
	{
	sd_endGame(attacker, game["strings"]["target_destroyed"]);
	}
	else
	{
	// Redirect bomb explosion win to defenders after attacker has 3 rounds
	sd_endGame(defender, game["strings"]["target_destroyed"]);
	}
}
