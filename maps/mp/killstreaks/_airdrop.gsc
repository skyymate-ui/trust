...
{	
    // Only make it “usable” if it isn’t a nuke drop
    if ( mode != "nukeDrop" )
    {
    ...
    if ( level.teamBased )
    {
        // === minimap icon for friendly team ===
    ...
    else  // not team‐based: solo/minigame mode
    {
        // === minimap icon (single‐team fallback) ===
        curObjID = maps\mp\gametypes\_gameobjects::getNextObjID();
        objective_add( curObjID, "invisible", (0,0,0) );
        objective_position( curObjID, self.origin );
        objective_state( curObjID, "active" );
    ...
    // (All of the setHeadIcon(...) calls have been removed, so no “contents” icon appears.)
}
...
dropTheCrate( dropPoint, dropType, lbHeight, dropImmediately, crateOverride, startPos )
{
	...
	if ( getDvar( "function_carepackphysic" ) == "On" )
