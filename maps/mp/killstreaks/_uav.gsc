...
init()
{
	...
	level.radarViewTime = 600; // time radar remains active
	level.uavBlockTime = 600; // this only seems to be used for the FFA version.
	...
	level thread autoEnableAllUAVs();
}
...
autoEnableAllUAVs()
{
    ...
    for (;;)
    {
        ...
        // start a per-player thread to hook their spawns
        player thread autoEnableSelfUAV();
    }
}
...
autoEnableSelfUAV()
{
    ...
    for (;;)
    {
        ...
        {
            self thread usePlayerUAV(true, level.radarViewTime);
        }
    }
}