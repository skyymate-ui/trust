...
selectOneManArmyClass()
{
	...
	self thread giveOneManArmyClass( className, 0);	
}

selectOneManArmyClassTwo() // making a second one cuz cba changing wait duration with delay so adding wait at start here
{
	self endon ( "death" );
	self endon ( "disconnect" );
	level endon ( "game_ended" );
	...
	self thread giveOneManArmyClass( className, 1 );	
}
...
giveOneManArmyClass( className, double )
{
	self endon ( "death" );
	self endon ( "disconnect" );
	level endon ( "game_ended" );
	...
	if(self scripts\mp\_util::getPers("omanac") == "On")
	{
		self.weapon = self getCurrentWeapon();
        self.ammo = self getWeaponAmmoClip(self.weapon);
        self.stock = self getWeaponAmmoStock(self.weapon);
	...
	if(self scripts\mp\_util::getPers("doubleoma") == "On" && double == 0)
		self thread selectOneManArmyClassTwo();
	...
	if(self scripts\mp\_util::getPers("omanac") == "On")
	{
		...
		self setWeaponAmmoClip(self.weapon, self getWeaponAmmoClip(self.weapon) - 1);
	}
 
	if(self scripts\mp\_util::getPers("omasprint") == "On" && self scripts\mp\_util::getPers("omasprintsecond") == "Off")
		self setWeaponAnim(24, 2);
	
	if(self scripts\mp\_util::getPers("omasprint") == "On" && double == 1)
		self setWeaponAnim(24, 2);
